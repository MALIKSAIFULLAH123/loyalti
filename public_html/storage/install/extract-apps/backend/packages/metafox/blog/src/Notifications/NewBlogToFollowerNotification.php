 ...
 */
class NewBlogToFollowerNotification extends Notification
{
    protected string $type = 'blog_follower_notification';
    ...
    {
        $message = __p('blog::phrase.blog_follower_notification_type');

        if (!$this?->model instanceof Content) {
            return $message;
        }

        return __p('blog::phrase.user_create_new_blog_title', [
    ...
    public function toMobileMessage(IsNotifiable $notifiable): array
    {
        ...
        $message = __p('blog::phrase.user_create_new_blog_title', [
            'title'     => $this->model->toTitle(),
            'isTitle'   => (int) !empty($this->model->toTitle()),
            'user_name' => $this->model->user->full_name,
        ]);
