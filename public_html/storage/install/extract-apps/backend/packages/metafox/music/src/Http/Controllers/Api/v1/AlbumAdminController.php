...
use MetaFox\Music\Http\Requests\v1\Album\Admin\BatchUpdateRequest;
use MetaFox\Music\Http\Requests\v1\Album\Admin\IndexRequest;
use MetaFox\Music\Http\Resources\v1\Album\Admin\AlbumItemCollection as ItemCollection;
 ...
 * | @link \MetaFox\Music\Http\Controllers\Api\AlbumAdminController::$controllers;
 */
...
class AlbumAdminController extends ApiController
{
    ...
    public function index(IndexRequest $request): ItemCollection
    {
        ...
        $data = $this->repository->viewAlbums($context, $params)->paginate($limit, ['music_albums.*']);
    ...
    public function sponsor(SponsorRequest $request, int $id): JsonResponse
    {
        ...
        return $this->success([], [], __p($message, ['resource_name' => __p('core::web.music_album')]));
    }
    ...
    public function batchDelete(BatchUpdateRequest $request): JsonResponse
    {
        ...
        $query->whereIn('id', $ids)
            ->get()
            ->each(function (Album $model) {
                $model->delete();
            });

        return $this->success([], [], __p('music::phrase.album_s_deleted_successfully'));
    }
}
