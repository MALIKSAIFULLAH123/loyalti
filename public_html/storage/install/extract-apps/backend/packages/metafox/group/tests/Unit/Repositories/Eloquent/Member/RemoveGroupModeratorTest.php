...
class RemoveGroupModeratorTest extends TestCase
{
    ...
    public function testSuccess(MemberRepositoryInterface $repository): array
    {
        ...
        Member::factory()->setUser($user2)->setOwner($group)->setModerator()->create();

        $group->refresh();
        $totalMember = $group->total_member;
        $this->assertTrue($group->isModerator($user2));

        $result = $repository->removeGroupModerator($user, $group->entityId(), $user2->entityId(), false);
        $group->refresh();
     ...
     * @throws AuthorizationException
     */
    public function testDeleteModerator(array $data)
    {
        /**
         * @var User                      $user
         * @var Group                     $group
        ...
        $result = $repository->removeGroupModerator($user, $group->entityId(), $user2->entityId(), true);
        $group->refresh();
        $this->assertTrue($result);
     ...
     * @throws AuthorizationException
     */
    public function testIsNotModerator(array $data)
    {
        /**
         * @var User                      $user
         * @var Group                     $group
        ...
        $repository->removeGroupModerator($user, $group->entityId(), $user2->entityId(), true);
    }
}
