...
const name = 'ProgressItem';

const ChatProgressItem = styled('div', {
  name,
  slot: 'root',
  shouldForwardProp: props => props !== 'isAllPage' && props !== 'isMobile'
})<{ isAllPage?: boolean; isMobile?: boolean }>(
  ({ theme, isAllPage, isMobile }) => ({
    '&:not(:first-of-type)': {
...
const ItemInfo = styled('div', { name, slot: 'wrap-item' })(({ theme }) => ({
  display: 'flex',
  alignItems: 'center',
  '& .ico': {
...
function ChatFileProgressItem({
  ...
  return (
    <ChatProgressItem isAllPage={isAllPage} isMobile={isMobile}>
      <ItemInfo>
