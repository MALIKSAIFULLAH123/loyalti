...
const BottomContainer = styled('div', {
  name,
  slot: 'bottom',
  shouldForwardProp: props => props !== 'isMinimize'
})<{ isMinimize?: boolean }>(({ theme, isMinimize }) => ({
  width: '100%',
  padding: theme.spacing(0, isMinimize ? 1 : 2),
  maxHeight: '65%',
  backgroundImage:
...
const VolumeSliderWrapper = styled(Box, {
  ...
  shouldForwardProp: props => props !== 'isHover' && props !== 'isMinimize'
})<{ isHover?: boolean; isMinimize?: boolean }>(
  ({ theme, isHover, isMinimize }) => ({
    height: 70,
    visibility: 'hidden',
    opacity: 0,
...
const InnerControls = styled('div', {
  ...
  shouldForwardProp(prop: string) {
    return !/visible|isMinimize/i.test(prop);
  }
})<{ visible: boolean; isMinimize?: boolean }>(
  ({ theme, visible, isMinimize }) => ({
    display: 'flex',
    ...(!isMinimize && {
...
const Control = (props: ControlProps) => {
  ...
  return (
    <RootControl ref={controlRef}>
      <BottomContainer isMinimize={isMinimize}>
        <ControlBox>
          <InnerControls visible={visible} isMinimize={isMinimize}>
            ...
            <SliderContainer isMinimize={isMinimize}>
              {isMinimize ? null : (
                <TimeSpan>
