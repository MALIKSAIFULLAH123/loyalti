...
export default class WsClient {
  ...
  public connect(context: any, onLoggedIn: DdpCallback): void {
    ...
    // currently just need socket when send streaming. not need keep continuous connection or try re-connect when close
  ...
  public loginWithResume(resume: string): Promise<any> {
  ...
  public loginWithAccessToken(accessToken: string): Promise<any> {
    return new Promise((resolve, reject) => {
      this.useMethod({
        name: '__oauthLogin',
  ...
  public login(resume: string, useCached: boolean = true): Promise<any> {
  ...
  private onPing({ id }): void {
    this._send(id ? { msg: 'pong', id } : { msg: 'pong' });
  }

  private onChanged({ collection, fields: { args, eventName } }): void {
    if (eventName) {
  ...
  private onResult({ error, id, result }: ResultType): void {
