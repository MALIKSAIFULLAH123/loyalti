...
import { InvoiceDetailViewProps as ItemProps } from '@ft/sevent/types';
import {
  DotSeparator,
  FormatDate,
  Image,
...
export default function DetailView({
  ...
  const { title, image, link } = seventItem || {};
  const imageSrc = getImageSrc(image, '240', assetUrl('sevent.no_image'));

  const handleBack = () => {
    if (seller?.id === userAuth?.id) {
      navigate('/sevent/invoice-sold');

      return;
    }

    navigate('/sevent/invoice-bought');
  };
...
DetailView.displayName = 'SeventDetailView';
