/**
 * @type: itemView
 * name: sevent_invoice.itemView.invoice
 */
...
const BoughtInvoice = ({ identity, wrapAs, wrapProps }: any) => {
  ...
  console.log(item);
  const ticket = useGetItem(item.ticket);
      ...
      <ItemWrapperStyled>
        ...
        <RowStyled>
          <TitleRowStyle variant="h5">
            {i18n.formatMessage({ id: 'sevent_campaign' })}
          </TitleRowStyle>
          <ValueRowStyled>
        ...
        <RowStyled>
          <TitleRowStyle variant="h5">
            {i18n.formatMessage({ id: 'sevent_qty' })}
          </TitleRowStyle>
          <ValueRowStyled>
            <TruncateText lines={2} sx={{ pr: 2 }}>
              {item.qty ? item.qty : ''}
