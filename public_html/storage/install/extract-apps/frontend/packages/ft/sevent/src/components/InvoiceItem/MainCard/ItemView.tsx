...
import { SeventInvoiceItemProps } from '@ft/sevent/types';
import {
  DotSeparator,
  FormatDate,
  ItemMedia,
...
export default function SeventInvoiceItemMainCard({
  item,
  identity,
  itemProps,
  user,
  ...
  wrapProps,
  pagingId
}: SeventInvoiceItemProps) {
  const { assetUrl, jsxBackend, useGetItem } = useGlobal();
  const seventItem = useGetItem(item?.sevent);
  ...
  const { title, image } = seventItem || {};

  const imageSrc = getImageSrc(image, '240', assetUrl('sevent.no_image'));
  const to = link || `/sevent/invoice/${item.id}`;
...
SeventInvoiceItemMainCard.displayName = 'SeventInvoiceItem(MainCard)';
