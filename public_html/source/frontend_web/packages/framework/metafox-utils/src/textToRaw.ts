...
const getOffset = (text, item) => {
  // parse emoji to 1 length for compataible with draftjs
  const updated = text.replace(emojiReg, '.');

  return updated.indexOf(item);
};
...
export default function textToRaw(text: string): RawDraftContentState {
  ...
  if (Array.isArray(listMention) && listMention.length > 0) {
    listMention.forEach((item, index) => {
      ...
      const offsetInDraftjs = getOffset(text, item);
