...
Route::get('sevent/category/{id}/category/browse', function ($id) {
    return seo_sharing_view(
        'admin',
        'admin.sevent.browse_child_category',
        'sevent_category',
        $id,
        function ($data, $resource) use ($id) {
            $data->addBreadcrumb(__p('core::phrase.categories'), '/sevent/category/browse');
            /**
             * @var \Foxexpert\Sevent\Models\Category $resource
             */
...
Route::get('sevent/add', function (Request $request) {
    return seo_sharing_view(
        'web',
        'sevent.sevent.create',
        null,
        null,
        function ($data, $resource) use ($request) {
            ...
            $data->addBreadcrumb(__p('sevent::phrase.label_menu_s'), sprintf('%s/%s', $ownerLink, 'sevent'));
        }
    );
});

Route::get('sevent/edit/{id}', function ($id) {
    return seo_sharing_view(
        'web',
        'sevent.sevent.edit',
        'sevent',
