...
class TranslatableTextSearchScope extends SearchScope
{
    ...
    public function apply(Builder $builder, Model $model)
    {
        ...
        $builder->leftJoin('phrases as ps', function (JoinClause $leftJoin) use ($locale, $table) {
            ...
            ->where(function (Builder $builder) use ($locale, $search, $table) {
                $builder->where(function (Builder $builder) use ($locale, $search, $table) {
                        $builder->where("$table.name", $this->likeOperator(), "%$search%")
    ...
    public function applyQueryBuilder(QueryBuilder $builder): void
    {
        ...
        $builder->leftJoin('phrases as ps', function (JoinClause $leftJoin) use ($locale, $table) {
            ...
            ->where(function (QueryBuilder $builder) use ($locale, $search, $table) {
                $builder->where(function (QueryBuilder $builder) use ($locale, $search, $table) {
                        $builder->where("$table.name", $this->likeOperator(), "%$search%")
                            ->whereNot("$table.name", $this->likeOperator(), '%::%.%');
                    })->orWhere(fn (QueryBuilder $subWhere) => $subWhere->where('ps.text', $this->likeOperator(), "%$search%"));
            });
