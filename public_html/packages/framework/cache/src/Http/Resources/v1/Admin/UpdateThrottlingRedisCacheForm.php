 ...
 * @driverName redis.throttling
 */
class UpdateThrottlingRedisCacheForm extends Form
{
    protected function prepare(): void
    {
        $res    = $this->resource ?? [];
        $value  = config('database.redis.throttling', []);
    ...
    public function validated(Request $request): array
    {
        ...
        config()->set('database.redis.throttling', $connectionConfig);

        $this->validateCacheConfiguration($connectionConfig);

        $data['driver']     = config('cache.stores.throttling.driver');
        $data['connection'] = 'throttling';
        $data['path']       = storage_path('framework/cache/throttling');
        $data['selectable'] = false;

        Settings::updateSetting(
            'cache',
            'cache.redis.throttling',
            'database.redis.throttling',
            null,
            $connectionConfig,
            'array',
            false,
