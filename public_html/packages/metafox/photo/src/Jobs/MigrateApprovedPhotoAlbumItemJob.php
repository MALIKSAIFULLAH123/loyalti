...
class MigrateApprovedPhotoAlbumItemJob extends AbstractJob
{
    use Dispatchable;
     ...
     * @param int  $page
     * @param int  $limit
     * @param bool $refreshBeforeStarting
     */
    ...
    public function handle()
    {
        ...
        $items = AlbumItem::query()
            ->with(['detail'])
            ->simplePaginate($this->limit, ['photo_album_item.*'], 'page', $this->page);
        ...
        $pendingIds = collect($items)
            ->filter(function (AlbumItem $item) {
                if (null === $item->detail) {
                    return false;
                }
