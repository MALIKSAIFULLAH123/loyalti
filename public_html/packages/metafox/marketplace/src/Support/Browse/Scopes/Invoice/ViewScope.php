...
namespace MetaFox\Marketplace\Support\Browse\Scopes\Invoice;
    ...
    public function apply(Builder $builder, Model $model)
    {
        ...
        switch ($view) {
            case self::VIEW_BOUGHT:
                ...
                break;
            case self::VIEW_SOLD:
                $builder->where('user_id', '<>', $context->entityId())
                    ->whereHas('listing', function (Builder $q) use ($context) {
                        $q->where('user_id', '=', $context->entityId())
                            ->withTrashed();
                    });
                break;
        }
