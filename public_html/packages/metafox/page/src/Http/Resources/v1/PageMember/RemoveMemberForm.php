...
use MetaFox\Page\Http\Requests\v1\PageMember\RemoveMemberFormRequest;
...
class RemoveMemberForm extends AbstractForm
{
    ...
    public function boot(RemoveMemberFormRequest $request, PageRepositoryInterface $pageRepository): void
    {
    ...
    {
        $this->action('page-member/remove-page-member')
            ->asDelete()
            ->title(__p('page::phrase.remove_member_confirm_title'))
            ->setValue([
                'page_id'           => $this->pageId,
                'user_id'           => $this->userId,
                'delete_activities' => 0,
    ...
    protected function initialize(): void
    {
        ...
        $basic->addFields(
            Builder::typography()
                ->color('text.hint')
                ->plainText(__p('page::phrase.remove_member_confirm_desc', [
