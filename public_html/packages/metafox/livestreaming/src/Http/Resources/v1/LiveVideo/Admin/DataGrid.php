...
class DataGrid extends Grid
{
    ...
    protected string $resourceName = 'live-video';
    ...
    protected function initialize(): void
    {
        ...
        $this->addColumn('duration')
            ->header(__p('livestreaming::phrase.duration'))
            ->width(200);

        $this->addColumn('is_streaming')
            ->header(__p('livestreaming::phrase.live_streaming'))
            ->asYesNoIcon()
            ->width(150, 150);
        ...
        $this->withExtraData([
            'show_total'        => true,
            'total_item_phrase' => 'total_value_live_videos',
        ]);
    }
    ...
    protected function getFeaturedActionMenu(ItemActionMenu|Actions $actionMenu): void
    {
        if ($actionMenu instanceof Actions) {
            $actionMenu->add('feature')
                ->asPatch()
                ->apiParams(['feature' => 1])
                ->apiUrl("{$this->resourceName}/feature/:id");
    ...
    protected function getSponsoredActionMenu(ItemActionMenu|Actions $actionMenu): void
    {
        ...
        if ($actionMenu instanceof ItemActionMenu) {
            $actionMenu->addItem('sponsor')
                ->action('sponsor')
                ->value(MetaFoxForm::ACTION_ADMINCP_BATCH_ITEM)
                ->label(__p('livestreaming::phrase.sponsor_this_item'))
                ->reload()
                ->showWhen([
                    'and',
            ...
            $actionMenu->addItem('unsponsor')
                ->action('unsponsor')
                ->value(MetaFoxForm::ACTION_ADMINCP_BATCH_ITEM)
                ->label(__p('livestreaming::phrase.unsponsor_this_item'))
                ->reload()
                ->showWhen([
                    'and',
    ...
    protected function getSponsoredInFeedActionMenu(ItemActionMenu|Actions $actionMenu): void
    {
        ...
        if ($actionMenu instanceof ItemActionMenu) {
            $actionMenu->addItem('sponsorInFeed')
                ...
                ->value(MetaFoxForm::ACTION_ADMINCP_BATCH_ITEM)
                ->label(__p('livestreaming::phrase.sponsor_in_feed'))
                ->reload()
                ->showWhen([
                    'and',
            ...
            $actionMenu->addItem('unsponsorInFeed')
                ...
                ->value(MetaFoxForm::ACTION_ADMINCP_BATCH_ITEM)
                ->label(__p('livestreaming::phrase.un_sponsor_in_feed'))
                ->reload()
                ->showWhen([
                    'and',
    ...
    protected function getApproveActionMenu(ItemActionMenu|Actions $actionMenu): void
    {
        if ($actionMenu instanceof Actions) {
            $actionMenu->add('approve')
                ->asPatch()
                ->apiUrl("admincp/{$this->appName}/{$this->resourceName}/approve/:id");
        }
