...
class MigrateTotalFollowForUserJob extends AbstractJob implements ShouldBeUnique
{
    use Dispatchable;
    ...
    private function updateValuesByPostgres(): void
    {
        DB::statement("
            ...
            SET total_follower = (
                ...
                join user_entities as ue on activity_subscriptions.user_id = ue.id
                WHERE activity_subscriptions.owner_id = users.id
                  AND activity_subscriptions.special_type is null
                  AND activity_subscriptions.is_active = true
                  AND activity_subscriptions.user_id != users.id
                  AND ue.entity_type != 'group'
        ...
        DB::statement("
            ...
            SET total_following = (
                SELECT COUNT(*)
                FROM activity_subscriptions
                join user_entities as ue on activity_subscriptions.owner_id = ue.id
                WHERE activity_subscriptions.user_id = users.id
                  AND activity_subscriptions.special_type is null
                  AND activity_subscriptions.is_active = true
                  AND activity_subscriptions.owner_id != users.id
                  AND ue.entity_type != 'group'
                  AND ue.id is not null
                )");
    }
    ...
    private function updateValuesByMysql(): void
    {
        DB::statement('UPDATE `users`
            ...
            ON k.owner_id = users.id
            SET `users`.total_follower = k.total_follower - 1');
        ...
        DB::statement('UPDATE `users`
            ...
            SET `users`.total_following = k.total_following - 1');
