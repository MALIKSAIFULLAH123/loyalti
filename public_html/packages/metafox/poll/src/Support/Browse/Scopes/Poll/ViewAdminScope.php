    ...
    public function apply(Builder $builder, Model $model)
    {
        ...
        switch ($view) {
            ...
            case self::VIEW_CLOSED:
                $builder->whereNotNull('polls.closed_at')
                    ->where('polls.closed_at', '<', now());
                break;
            case Browse::VIEW_FEATURE:
                $builder->where('polls.is_featured', '=', 1);
                break;
            case Browse::VIEW_SPONSOR:
                $builder->where('polls.is_sponsor', '=', 1);
                break;
        }
    }
}
