...
class DataGrid extends Grid
{
    ...
    protected function initialize(): void
    {
        ...
        $this->addColumn('question')
            ->header(__p('poll::phrase.poll_question'))
            ->linkTo('url')
            ->target('_blank')
            ->flex();
        ...
        $this->addColumn('close_time')
            ->header(__p('poll::phrase.close_time'))
            ->asDateTime()
            ->width(250);
        ...
        $this->withExtraData([
            'show_total'        => true,
            'total_item_phrase' => 'total_value_polls',
        ]);
    ...
    protected function getSponsoredInFeedActionMenu(ItemActionMenu|Actions $actionMenu): void
    {
        ...
        if ($actionMenu instanceof ItemActionMenu) {
            $actionMenu->addItem('sponsorInFeed')
                ...
                ->value(MetaFoxForm::ACTION_ADMINCP_BATCH_ITEM)
                ->label(__p('event::phrase.sponsor_in_feed'))
                ->reload()
                ->showWhen([
                    'and',
            ...
            $actionMenu->addItem('unsponsorInFeed')
                ...
                ->value(MetaFoxForm::ACTION_ADMINCP_BATCH_ITEM)
                ->label(__p('event::phrase.un_sponsor_in_feed'))
                ->reload()
                ->showWhen([
                    'and',
