...
class RemoveLinkPreviewTest extends TestCase
{
     ...
     * @return Comment
     */
    public function testCreateCommentSuccess(User $user, Content $content, CommentRepositoryInterface $repository): Comment
    {
        $this->actingAs($user);
     ...
     * @return void
     */
    public function testPossibleToRemoveLinkPreview(User $user, Comment $comment)
    {
        $this->actingAs($user);

        $result = $user->can('removeLinkPreview', [Comment::class, $comment]);
     ...
     * @return void
     */
    public function testImpossibleToRemoveLinkPreview(User $user, Comment $comment)
    {
        $result = $user->can('removeLinkPreview', [Comment::class, $comment]);
