...
class HostInviteObserver
{
    public function created(Model $model): void
    {
        $event = $model->event;
        if ($model->isPending()) {
            $event->incrementAmount('total_pending_host_invite');
        }
    }
    ...
    public function deleted(Model $model): void
    {
        ...
        if ($model->isPending()) {
            $event->decrementAmount('total_pending_host_invite');
        }
    }
    ...
    public function updated(Model $model): void
    {
        ...
        if ($model->isDirty(['status_id'])) {
            ...
            if ($newStatus == Model::STATUS_APPROVED) {
                $event->incrementAmount('total_host');
            }
